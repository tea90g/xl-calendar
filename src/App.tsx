...
const UPDATE_NOTICE_STORAGE_KEY = "xl-calendar-last-seen-update-version-1.2.7-holiday-audit";
const UPDATE_NOTES_BY_VERSION = {
  ...
  "1.2.7": [
    "한국 공휴일 데이터를 2025~2030년 전체 재검토하여 누락 여부 점검",
    "일본 祝日·休日 데이터를 2025~2030년 전체 재검토하여 대체휴일·샌드위치 휴일 누락 수정",
    "2025년 2월 24일·5월 6일·11월 24일, 2026년 5월 6일·9월 22일, 2027년 3월 22일, 2029년 2월 12일·4월 30일·9월 24일, 2030년 5월 6일·8월 12일·11월 4일 등 일본 休日 추가",
  ].map((item) => `• ${item}`).join("\n"),
};
...
const JP_HOLIDAYS = {
  2025: [["2025-01-01", "元日"], ["2025-01-13", "成人の日"], ["2025-02-11", "建国記念の日"], ["2025-02-23", "天皇誕生日"], ["2025-02-24", "振替休日"], ["2025-03-20", "春分の日"], ["2025-04-29", "昭和の日"], ["2025-05-03", "憲法記念日"], ["2025-05-04", "みどりの日"], ["2025-05-05", "こどもの日"], ["2025-05-06", "振替休日"], ["2025-07-21", "海の日"], ["2025-08-11", "山の日"], ["2025-09-15", "敬老の日"], ["2025-09-23", "秋分の日"], ["2025-10-13", "スポーツの日"], ["2025-11-03", "文化の日"], ["2025-11-23", "勤労感謝の日"], ["2025-11-24", "振替休日"]],
  2026: [["2026-01-01", "元日"], ["2026-01-12", "成人の日"], ["2026-02-11", "建国記念の日"], ["2026-02-23", "天皇誕生日"], ["2026-03-20", "春分の日"], ["2026-04-29", "昭和の日"], ["2026-05-03", "憲法記念日"], ["2026-05-04", "みどりの日"], ["2026-05-05", "こどもの日"], ["2026-05-06", "振替休日"], ["2026-07-20", "海の日"], ["2026-08-11", "山の日"], ["2026-09-21", "敬老の日"], ["2026-09-22", "休日"], ["2026-09-23", "秋分の日"], ["2026-10-12", "スポーツの日"], ["2026-11-03", "文化の日"], ["2026-11-23", "勤労感謝の日"]],
  2027: [["2027-01-01", "元日"], ["2027-01-11", "成人の日"], ["2027-02-11", "建国記念の日"], ["2027-02-23", "天皇誕生日"], ["2027-03-21", "春分の日"], ["2027-03-22", "振替休日"], ["2027-04-29", "昭和の日"], ["2027-05-03", "憲法記念日"], ["2027-05-04", "みどりの日"], ["2027-05-05", "こどもの日"], ["2027-07-19", "海の日"], ["2027-08-11", "山の日"], ["2027-09-20", "敬老の日"], ["2027-09-23", "秋分の日"], ["2027-10-11", "スポーツの日"], ["2027-11-03", "文化の日"], ["2027-11-23", "勤労感謝の日"]],
  ...
  2029: [["2029-01-08", "成人の日"], ["2029-02-11", "建国記念の日"], ["2029-02-12", "振替休日"], ["2029-02-23", "天皇誕生日"], ["2029-03-20", "春分の日"], ["2029-04-29", "昭和の日"], ["2029-04-30", "振替休日"], ["2029-05-03", "憲法記念日"], ["2029-05-04", "みどりの日"], ["2029-05-05", "こどもの日"], ["2029-07-16", "海の日"], ["2029-08-11", "山の日"], ["2029-09-17", "敬老の日"], ["2029-09-23", "秋分の日"], ["2029-09-24", "振替休日"], ["2029-10-08", "スポーツの日"], ["2029-11-03", "文化の日"], ["2029-11-23", "勤労感謝の日"]],
  2030: [["2030-01-14", "成人の日"], ["2030-02-11", "建国記念の日"], ["2030-02-23", "天皇誕生日"], ["2030-03-20", "春分の日"], ["2030-04-29", "昭和の日"], ["2030-05-03", "憲法記念日"], ["2030-05-04", "みどりの日"], ["2030-05-05", "こどもの日"], ["2030-05-06", "振替休日"], ["2030-07-15", "海の日"], ["2030-08-11", "山の日"], ["2030-08-12", "振替休日"], ["2030-09-16", "敬老の日"], ["2030-09-23", "秋分の日"], ["2030-10-14", "スポーツの日"], ["2030-11-03", "文化の日"], ["2030-11-04", "振替休日"], ["2030-11-23", "勤労感謝の日"]],
};
...
function makeLocalStorageState(source) {
  const electronAPI = getElectronStateApi();
  if (electronAPI) return source;

  // Mobile/web storage is small. Keep large base64 assets in Google Drive only.
  return {
    ...source,
    image: null,
    timerImages: { work: "", other: "", away: "" },
    stickers: (Array.isArray(source?.stickers) ? source.stickers : []).map((sticker) => ({
      ...sticker,
      src: "",
    })),
  };
...
function saveStateToLocalStorage(source) {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(makeLocalStorageState(source)));
    return true;
  } catch (err) {
    console.warn("[XL Calendar] localStorage save failed; retrying with minimal state", err);
    try {
      const minimal = makeLocalStorageState(source);
      minimal.stickers = [];
      minimal.image = null;
      minimal.timerImages = { work: "", other: "", away: "" };
      localStorage.removeItem(STORAGE_KEY);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(minimal));
      return true;
    } catch (retryErr) {
      console.error("[XL Calendar] minimal localStorage save failed", retryErr);
      return false;
    }
  }
}
...
export default function App() {
  ...
  function addStickerFromFile(file) {
    ...
    if ((state.stickers || []).length >= STICKER_MAX_COUNT) {
      alert("스티커는 PNG, JPG, WEBP, GIF 형식으로 최대 5개까지 사용할 수 있어요.");
      return;
    }
